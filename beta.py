...
import requests  # Добавлен для keep-alive
...
# ✅ ПОЛНЫЙ СЛОВАРЬ ключевых слов (ВСЕ возможные!)
KEYWORDS = [
    # Русский
...
# Keep-alive для Render (пингует каждые 14 минут)
def keep_alive():
    while True:
        try:
...
# ✅ ФУНКЦИЯ ЛОГИНА - ОТДЕЛЬНО ДЛЯ ВСЕХ СТРАНИЦ АДМИНА
    ...
    <div class="container">
        <div class="card">
            ...
            <div class="status">
                ...
                3️⃣ Когда будете оплачивать ведите ту сумму которая указана на странице (3146)<br>
            </div>
        </div>
    </div>
...
# ✅ ИСПРАВЛЕННАЯ АДМИНКА - РАБОТАЕТ ДЛЯ ВСЕХ СТРАНИЦ
@app.route('/orders', methods=['GET', 'POST'])
@app.route('/logs', methods=['GET', 'POST'])
def admin_panel():
    # Проверяем авторизацию для ЛЮБОЙ админ страницы
...
# ✅ РАСШИРЕННЫЙ парсер ключей (Steam + GOG + все форматы!)
...
Спасибо за покупку! 🚀"""
        ...
        server = smtplib.SMTPServer('smtp.gmail.com', 587)
        server.starttls()
...
if __name__ == '__main__':
    log_message("🚀 CYBERPUNK BOT v2.3 (ИСПРАВЛЕНА АДМИНКА + IMAP СТАТУС)")
